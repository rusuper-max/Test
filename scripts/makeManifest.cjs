...
// Dodaj kategorije koje zaista imaš kao foldere u public/portfolio
const CATS = [
...
for (const cat of CATS) {
  ...
  if (fs.existsSync(full)) {
    manifest[cat] = listFilesSafe(full);
  }
}

const outFile = path.join(ROOT, 'src', 'data', 'portfolioManifest.ts');
const file = `// ⚠️ AUTO-GENERATED by scripts/makeManifest.cjs — do not edit
